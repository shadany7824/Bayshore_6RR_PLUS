...
export default class CarModule extends Module {
    register(app: Application): void {
		...
		app.post('/method/create_car', async (req, res) => {
			...
			// THA regionId
			let regionId: number = 40;
			let country: string = "THA";
			...
			// Add Meter to new car
			let addMeter = Config.getConfig().gameOptions.grantMeterToNewcar;
			if (addMeter > 0) {
				console.log(`Give Meter to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addMeter; i++) {
			...
			let addNameplate = Config.getConfig().gameOptions.grantNameplateToNewcar;
			if (addNameplate > 0) {
				console.log(`Give Nameplate to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addNameplate; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_NAME_PLATE,
							itemId: i,
							amount: 1
						}
					});
			...
			let addCustomcolor = Config.getConfig().gameOptions.grantCustomcolorTonewCar;
			if (addCustomcolor > 0) {
				console.log(`Give Custom Colors to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addCustomcolor; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_CUSTOM_COLOR,
							itemId: i,
							amount: 1
						}
					});
			...
			let addBGM = Config.getConfig().gameOptions.grantBGMTonewCar;
			if (addBGM > 0) {
				console.log(`Give BGM to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addBGM; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_BGM,
							itemId: i,
							amount: 1
						}
					});
			...
			let addAerofullset = Config.getConfig().gameOptions.grantAerofullsetTonewCar;
			if (addAerofullset > 0) {
				console.log(`Give Aero Full set to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addAerofullset; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_AERO_FULLSET,
							itemId: i,
							amount: 1
						}
					});
			...
			let addAero = Config.getConfig().gameOptions.grantAeroTonewCar;
			if (addAero > 0) {
				console.log(`Give Aero to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addAero; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_AERO,
							itemId: i,
							amount: 1
						}
					});
			...
			let addBonnet = Config.getConfig().gameOptions.grantBonnetTonewCar;
			if (addBonnet > 0) {
				console.log(`Give Bonnet to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addBonnet; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_BONNET,
							itemId: i,
							amount: 1
						}
					});
			...
			let addWing = Config.getConfig().gameOptions.grantWingTonewCar;
			if (addWing > 0) {
				console.log(`Give Wing to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addWing; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_WING,
							itemId: i,
							amount: 1
						}
					});
			...
			let addWheel = Config.getConfig().gameOptions.grantWheelTonewCar;
			if (addWheel > 0) {
				console.log(`Give Wheel to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addWheel; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_WHEEL,
							itemId: i,
							amount: 1
						}
					});
			...
			let addMirror = Config.getConfig().gameOptions.grantMirrorTonewCar;
			if (addMirror > 0) {
				console.log(`Give Mirror to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addMirror; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_MIRROR,
							itemId: i,
							amount: 1
						}
					});
			...
			let addNeon = Config.getConfig().gameOptions.grantNeonTonewCar;
			if (addNeon > 0) {
				console.log(`Give Neon to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addNeon; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_NEON,
							itemId: i,
							amount: 1
						}
					});
			...
			let addTrunk = Config.getConfig().gameOptions.grantTrunkTonewCar;
			if (addTrunk > 0) {
				console.log(`Give Trunk to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addTrunk; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_TRUNK,
							itemId: i,
							amount: 1
						}
					});
			...
			let addNumberplate = Config.getConfig().gameOptions.grantNumberplateTonewCar;
			if (addNumberplate > 0) {
				console.log(`Give Number Plate to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addNumberplate; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
			...
			let addGTWing = Config.getConfig().gameOptions.grantGTWingTonewCar;
			if (addGTWing > 0) {
				console.log(`Give GT Wing Plate to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addGTWing; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_GT_WING,
							itemId: i,
							amount: 1
						}
					});
			...
			let addAuraVS = Config.getConfig().gameOptions.grantAuraVSTonewCar;
			if (addAuraVS > 0) {
				console.log(`Give Aura VS to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addAuraVS; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_AURA_MOTIF,
							itemId: i,
							amount: 1
						}
					});
			...
			let addWindowsDecoration = Config.getConfig().gameOptions.grantWindowsDecorationTonewCar;
			if (addWindowsDecoration > 0) {
				console.log(`Give Windows Decoration to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addWindowsDecoration; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
			...
			let addRivalMarker = Config.getConfig().gameOptions.grantRivalMarkerTonewCar;
			if (addRivalMarker > 0) {
				console.log(`Give Rival Marker to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addRivalMarker; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
			...
			let addStamp = Config.getConfig().gameOptions.grantStampTonewCar;
			if (addStamp > 0) {
				console.log(`Give Stamp to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addStamp; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
							category: wm.wm.protobuf.ItemCategory.CAT_STAMP,
							itemId: i,
							amount: 1
						}
					});
			...
			let addCustomframe = Config.getConfig().gameOptions.grantCustomframTonewCar;
			if (addCustomframe > 0) {
				console.log(`Give Custom Frame to new car ${car.name} with ID ${car.carId}`);
				for (let i = 1; i < addCustomframe; i++) {
					await prisma.carItem.create({
						data: {
							carId: car.carId,
		...
		app.post('/method/update_car', async (req, res) => {
			...
			if (cars)
			{
				...
				data = {
					...
					customFrame: common.sanitizeInput(cars.customFrame),
